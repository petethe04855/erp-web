...
export const useErpStore = create<CustomErpStore>((set, get) => {
	...
	deleteUser: async (id) => {
		...
		await readApiResponse<{ id: string }>(res)
	...
	createQuotation: (input) => {
		...
		fetch(`${getApiUrl()}/api/quotations`, {
			method: 'POST',
			headers: getHeaders(),
			// ID and code are generated by the API. The optimistic workflow uses a
			// display code as its temporary ID, which must not be sent to Go's uint ID.
			body: JSON.stringify({
				customer: quotation.customer,
				customerAddress: quotation.customerAddress,
				date: quotation.date,
				validUntil: quotation.validUntil,
				leadSource: quotation.leadSource,
				status: quotation.status,
				lines: quotation.lines.map(line => ({ sku: line.sku, qty: line.qty, price: line.price })),
			}),
		}).then(res => {
			if (res.ok) get().fetchInitialState()
		})
	...
	updateQuotationLeadSource: (id, leadSource) => {
		const updated = workflow.updateQuotationLeadSource(id, leadSource)
		fetch(`${getApiUrl()}/api/quotations/${id}/lead-source`, {
			method: 'PUT',
			headers: getHeaders(),
			body: JSON.stringify({ leadSource }),
		}).then(res => {
			if (res.ok) get().fetchInitialState()
		})
		return updated
