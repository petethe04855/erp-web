    ...
    <SlidePanel open={open} onClose={onClose} title="สร้าง Sales Order" subtitle="กรอกข้อมูลออร์เดอร์ขายใหม่"
      footer={
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '9px 20px', border: `1px solid ${c.border}`, borderRadius: 7, background: c.surface, cursor: 'pointer', fontSize: 13, color: c.ink2 }}>ยกเลิก</button>
          <button onClick={onSubmit} style={{ padding: '9px 20px', border: 'none', borderRadius: 7, background: c.accent, color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>บันทึก SO</button>
        </div>
      }
    >
      <div style={{ display: 'grid', gap: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: c.ink2 }}>
            วันที่
            <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} style={panelInput(c.surface, c.border, c.ink)} />
          </label>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: c.ink2 }}>
            ช่องทาง
            <select value={form.channel} onChange={e => setForm(f => ({ ...f, channel: e.target.value }))} style={panelInput(c.surface, c.border, c.ink)}>
              {CHANNELS.map(ch => <option key={ch}>{ch}</option>)}
            </select>
          </label>
          <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 600, color: c.ink2 }}>
            QT อ้างอิง
            <input value={form.qtRef} onChange={e => setForm(f => ({ ...f, qtRef: e.target.value }))} style={panelInput(c.surface, c.border, c.ink)} />
          </label>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: c.ink }}>รายการสินค้า</span>
          <Btn t={t} variant="ghost" onClick={addLine}>+ เพิ่มสินค้า</Btn>
        </div>
        {form.lines.map((line, i) => {
          const product = products.find(p => p.sku === line.sku)
          return (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 100px 28px', gap: 8, alignItems: 'center' }}>
              <select value={line.sku} onChange={e => updateLine(i, 'sku', e.target.value)} style={panelInput(c.surface, c.border, c.ink)}>
        ...
        <div style={{ padding: '12px 16px', background: c.subtle, borderRadius: 8, display: 'flex', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 13, color: c.ink2 }}>รวม</span>
          <Mono t={t} size={18} weight={600}>{fmtBaht(lineTotal)}</Mono>
        </div>
      </div>
    </SlidePanel>
  )
