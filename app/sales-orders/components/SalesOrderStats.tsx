    ...
    <Card t={t} pad={false} style={{ marginBottom: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
        {[
          ...
          <div key={item.label} style={{ padding: '16px 22px', borderRight: i < 3 ? `1px solid ${c.border}` : 'none' }}>
            <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.10em', textTransform: 'uppercase', color: c.ink3 }}>{item.label}</div>
            <Mono t={t} size={20} weight={600} style={{ display: 'block', marginTop: 8 }}>{item.value}</Mono>
            <div style={{ fontSize: 11, color: c.ink3, marginTop: 3 }}>{item.sub}</div>
          </div>
        ))}
      </div>
    </Card>
