...
function bomAvailabilityLabel(bom: BOMOption, products: Product[]) {
  const finishedProduct = products.find((product) => product.sku === bom.fgSku);
  const finishedStock = finishedProduct
    ? Math.max(0, finishedProduct.stock - finishedProduct.reservedQty)
    : 0;
  const productionCapacity =
    bom.maxProducibleQty ?? availableFromBom(bom, products);
  const unit = bom.outputUnit || finishedProduct?.baseUnit || "ถุง";

  return ` · สต็อกสำเร็จรูป ${finishedStock.toLocaleString()} ${unit} · ผลิตได้จาก BOM ${productionCapacity.toLocaleString()} ${unit}`;
}
...
export default function SalesOrderFormPanel({
  ...
  const sellableProducts = products.filter(
    (p) =>
      p.type === "Finished Product" ||
      p.type === "Bundle" ||
      p.type === "Cat" ||
      p.type === "Dog",
  );
  const bomSkus = new Set(boms.map((bom) => bom.fgSku).filter(Boolean));
  const directProducts = sellableProducts.filter(
    (product) => !bomSkus.has(product.sku),
  );
      ...
      lines: f.lines.map((line, idx) =>
        idx === i
          ? {
              ...
              bomUnitCost: bom
                ? bom.cost / Math.max(bom.outputQty || 1, 1)
                : undefined,
              bomAvailableQty,
            }
          : line,
  ...
  return (
    <SlidePanel
      ...
      footer={
        <div className="flex gap-2.5 justify-end">
          ...
          <Button
            onClick={onSubmit}
            className="bg-[var(--erp-accent)] text-white hover:opacity-90 cursor-pointer shadow-none border-none"
          >
            บันทึก SO
          </Button>
        </div>
    ...
    >
      <div className="grid gap-4">
          ...
          return (
            <div key={i} className="space-y-1">
              <div className="grid grid-cols-[1fr_80px_100px_28px] gap-2 items-center">
                <NativeSelect
                  value={line.bomId ? `${line.sku}::${line.bomId}` : line.sku}
                  onChange={(e) => selectProduct(i, e.target.value)}
                  className="w-full cursor-pointer"
                >
                  <option value="">-- เลือกสินค้า --</option>
                  {boms
                    .filter((bom) => bom.fgSku && bom.status === "Active")
                    .map((bom) => (
                      <option
                        key={`bom-${bom.id}`}
                        value={`${bom.fgSku}::${bom.id}`}
                      >
                        {bom.name} ({bom.fgSku}) · ผลผลิตมาตรฐาน:{" "}
                        {bom.outputQty} {bom.outputUnit || "ชิ้น"} · {bom.code}
                        {bomAvailabilityLabel(bom, products)}
                      </option>
                    ))}
                ...
                {line.bomAvailableQty !== undefined && (
                  <div className="col-span-4 -mt-1 text-right text-[11px] text-muted-foreground">
                    BOM ผลิต/ขายได้สูงสุด{" "}
                    {line.bomAvailableQty.toLocaleString()} ชิ้น
                  </div>
                )}
                {form.lines.length > 1 && (
                  <Button
              ...
              {selectedBom && (
                <div className="text-[11px] text-emerald-700 pl-1 font-medium">
                  🧩 สูตร {selectedBom.code}: ผลผลิตมาตรฐาน{" "}
                  {selectedBom.outputQty} {selectedBom.outputUnit || "ชิ้น"} ·
                  ต้นทุนต่อหน่วย {fmtBaht2(line.bomUnitCost || 0)}
                </div>
              )}
            </div>
          );
        ...
        <div
          className="p-3 bg-muted rounded-lg flex justify-between mt-2"
