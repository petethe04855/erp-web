  ...
  return (
    <div style={{ minHeight: '100vh', background: c.canvas }}>
      <TopBar
        ...
        right={
          <>
            ...
            <Button onClick={() => { setForm(BLANK); setOpen(true) }}>+ New PO</Button>
          </>
        }
      />
                  ...
                  <TableRow key={po.id}>
                    <TableCell className="py-3.5 px-6">
                      <div className="flex items-center gap-2">
                        ...
                        {po.status === 'Draft' && (
                          <Button variant="outline" onClick={() => handleStatusChange(po.id, 'Sent')} className="h-6 px-2 text-[10px]">
                            Send
                          </Button>
                        )}
                      </div>
                    ...
                    <TableCell className="py-3.5 px-6">
                      <span style={{ fontSize: 13, fontWeight: 500, color: c.ink }}>{po.supplier}</span>
                    </TableCell>
                    <TableCell className="py-3.5 px-6">
                      <Mono t={t} size={12} color={c.ink2}>{po.date}</Mono>
                    </TableCell>
                    <TableCell className="py-3.5 px-6">
                      <Mono t={t} size={12} color={c.ink2}>{po.etaDate}</Mono>
                    </TableCell>
                    <TableCell className="py-3.5 px-6 text-right">
                      <Mono t={t} size={12} color={c.ink2}>{po.items.length}</Mono>
                    </TableCell>
                    <TableCell className="py-3.5 px-6 text-right">
                      <Mono t={t} size={13} weight={600}>{formatBaht(po.totalCost)}</Mono>
                    </TableCell>
                    <TableCell className="py-3.5 px-6">
                      {getBadge(po.status)}
                    </TableCell>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="sm:max-w-[620px] flex flex-col h-full p-0 bg-background">
          <SheetHeader className="p-6 border-b border-border flex-shrink-0">
            <SheetTitle className="text-base font-bold text-foreground">New Purchase Order</SheetTitle>
            <SheetDescription className="text-xs text-muted-foreground mt-1">สร้างใบสั่งซื้อใหม่ - ยอดรวม {formatBaht(lineTotal)}</SheetDescription>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div className="grid gap-2">
              ...
              <Input type="date" value={form.etaDate} onChange={e => setForm(f => ({ ...f, etaDate: e.target.value }))} />
            </div>
                  ...
                  {form.items.map((line, i) => (
                    <tr key={i} style={{ borderBottom: i === form.items.length - 1 ? 'none' : `1px solid ${c.border}` }}>
                      ...
                      <td style={{ padding: 10, width: 116 }}>
                        <Input type="number" min={0} value={line.unitCost} onChange={e => updateLine(i, 'unitCost', parseFloat(e.target.value) || 0)} className="text-right" />
                      </td>
          ...
          <SheetFooter className="border-t border-border p-6 flex-shrink-0">
            <div className="flex justify-end gap-2 w-full">
              ...
              <Button onClick={handleSubmit} className="bg-[#0F6E58] text-white hover:bg-[#0F6E58]/90">Save PO</Button>
            </div>
          </SheetFooter>
        </SheetContent>
      </Sheet>
