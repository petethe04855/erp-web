...
const MODULE_SECTIONS: Array<{
  ...
  },
  {
    ...
    items: [
      ...
      {
        key: "tiktokCalculator",
        label: "TikTok Calculator",
        desc: "คำนวณค่าธรรมเนียม",
      },
...
export default function SettingsPage() {
  ...
  function toggleNotif(key: keyof typeof settings.notifications) {
    ...
    updateSettings({
      notifications: { ...n, [key]: !n[key as keyof typeof n] },
    });
  }
        ...
        <Card t={t} style={{ padding: "24px 28px" }}>
            ...
            <div>
              ...
              >
                {(
                  ...
                  ] as Array<{ field: keyof typeof company; label: string }>
                ).map(({ field, label }) => (
              ...
              >
                ...
                <Field t={t} label="VAT (%)">
                  <input
                    ...
                    onChange={(e) =>
                      setCompany((p) => ({ ...p, vatRate: e.target.value }))
                    }
                    style={inp}
                  />
                </Field>
              ...
              {[
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    {item.key === "nearExpiry" &&
                      settings.notifications.nearExpiry && (
                        <div
                          style={{
                            display: "flex",
                            alignItems: "center",
                  ...
                  <Toggle
                    t={t}
                    on={Boolean(settings.notifications[item.key])}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {categories.map((cat, i) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        setCategories((p) => p.filter((_, idx) => idx !== i))
                      }
                      style={{
                        background: "none",
                        border: "none",
          ...
          {activeTab === "reorder" && (
            <div>
              ...
              <div style={{ fontSize: 13, color: c.ink3, marginBottom: 16 }}>
                    ...
                    {products
                      ...
                      .map((p, i) => (
                        <tr
                          key={p.sku}
                          style={{
                            background: i % 2 === 0 ? c.canvas : c.surface,
                          }}
                        >
                          <td
                            style={{
                          ...
                          <td
                            style={{
                              ...
                              fontWeight: 700,
                              color:
                                p.stock <= (reorderDraft[p.sku] ?? p.reorder)
                                  ? c.neg
                                  : c.pos,
                            }}
                          >
                          ...
                          >
                            <input
                              ...
                              onChange={(e) =>
                                setReorderDraft((d) => ({
                                  ...d,
                                  [p.sku]: parseInt(e.target.value) || 0,
                                }))
                              }
                              style={{
                                width: 80,
          ...
          {activeTab === "modules" && (
            <div>
              ...
              >
                ปิด module แล้วข้อมูลยังอยู่ครบ — แค่ซ่อนออกจาก sidebar.
                Dashboard และ Settings เปิดเสมอ
              </div>
                    ...
                    {group.items.map((item, idx) => (
                      <div
                        ...
                        style={{
                          ...
                          alignItems: "center",
                          gap: 16,
                          background: settings.modules[item.key]
                            ? c.canvas
                            : c.surface,
                        }}
                      >
                        <div style={{ flex: 1 }}>
                          <div
                            style={{
                              fontSize: 13,
                              fontWeight: 500,
                              color: settings.modules[item.key]
                                ? c.ink
                                : c.ink3,
                            }}
                          >
          ...
          {activeTab === "livePayroll" && (
            <div>
              ...
              <div style={{ fontSize: 13, color: c.ink3, marginBottom: 20 }}>
                อัตราค่าแรงและโบนัสใช้เท่ากันสำหรับพนักงาน Live ทุกคน ไม่มี
                Commission
              </div>
              <div
                style={{
              ...
              >
                <Field
                  t={t}
                  label="Hourly Rate (฿/ชั่วโมง)"
                  sub="อัตราค่าแรงต่อชั่วโมง (เหมือนกันทุกคน)"
                >
                  <input
                    type="number"
                    min={0}
                    value={payrollDraft.hourlyRate}
                    onChange={(e) =>
                      setPayrollDraft((d) => ({
                        ...d,
                ...
                <Field
                  t={t}
                  label="Clip Bonus (฿/คลิป)"
                  sub="โบนัสต่อคลิปที่ตัด (เหมือนกันทุกคน)"
                >
                  <input
                    type="number"
                    min={0}
                    value={payrollDraft.clipBonus}
                    onChange={(e) =>
                      setPayrollDraft((d) => ({
                        ...d,
                        clipBonus: e.target.value,
                      }))
                    }
                    style={inp}
                  />
              ...
              >
                <strong>ตัวอย่าง:</strong> ทำงาน 2.5 ชั่วโมง, ตัด 1 คลิป →
                ค่าแรง ฿
                {(
                  2.5 * (parseFloat(payrollDraft.hourlyRate) || 0)
                ).toLocaleString()}{" "}
                + โบนัส ฿
                {(parseFloat(payrollDraft.clipBonus) || 0).toLocaleString()} ={" "}
                <strong>
                  ฿
                  {(
                    2.5 * (parseFloat(payrollDraft.hourlyRate) || 0) +
                    (parseFloat(payrollDraft.clipBonus) || 0)
                  ).toLocaleString()}
                </strong>
              </div>
            </div>
                ...
                {[
                  ...
                  >
                    ...
                    {section.items.map((item) => (
                      ...
                      >
                        <span style={{ color: c.pos, fontSize: 10 }}>✓</span>{" "}
                        {item}
                      </div>
                    ))}
                  </div>
