...
function ReportPagination({ total, page, onPage }: { total: number; page: number; onPage: (page: number) => void }) {
  if (total <= REPORT_PAGE_SIZE) return null;
  const pages = Math.ceil(total / REPORT_PAGE_SIZE);
  return <div className="flex items-center justify-between border-t p-3 text-xs text-muted-foreground"><span>แสดง {(page - 1) * REPORT_PAGE_SIZE + 1}–{Math.min(page * REPORT_PAGE_SIZE, total)} จาก {total} รายการ</span><div className="flex items-center gap-2"><button className="rounded border px-3 py-1 disabled:opacity-40" disabled={page <= 1} onClick={() => onPage(Math.max(1, page - 1))}>ก่อนหน้า</button><span>หน้า {page} / {pages}</span><button className="rounded border px-3 py-1 disabled:opacity-40" disabled={page >= pages} onClick={() => onPage(Math.min(pages, page + 1))}>ถัดไป</button></div></div>;
}
...
export default function ReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"revenue" | "trial" | "valuation" | "ledger">("revenue");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSummary(summaryData); setTrial(trialData); setValuation(valuationData); setLedger(ledgerData); setRevenue(revenueData); setRevenuePage(1); setTrialPage(1); setValuationPage(1); setLedgerPage(1);
  ...
  const filteredRevenue = (revenue?.rows || []).filter((row) => !reportSearch || `${row.reference} ${row.customer} ${row.channel}`.toLowerCase().includes(reportSearch.toLowerCase()));
  const filteredTrial = (trial?.rows || []).filter((row) => !reportSearch || `${row.AccountCode} ${row.AccountName} ${row.AccountType}`.toLowerCase().includes(reportSearch.toLowerCase()));
  const filteredValuation = (valuation?.rows || []).filter((row) => !reportSearch || `${row.SKU} ${row.ProductName} ${row.Lot}`.toLowerCase().includes(reportSearch.toLowerCase()));
  const reportPeriod = from || to ? `${from || "เริ่มต้น"} ถึง ${to || "ปัจจุบัน"}` : month;
  const visibleRevenueRows = filteredRevenue.slice((revenuePage - 1) * REPORT_PAGE_SIZE, revenuePage * REPORT_PAGE_SIZE);
  const visibleTrialRows = filteredTrial.slice((trialPage - 1) * REPORT_PAGE_SIZE, trialPage * REPORT_PAGE_SIZE);
  const visibleValuationRows = filteredValuation.slice((valuationPage - 1) * REPORT_PAGE_SIZE, valuationPage * REPORT_PAGE_SIZE);
  const visibleLedgerRows = filteredLedger.slice((ledgerPage - 1) * REPORT_PAGE_SIZE, ledgerPage * REPORT_PAGE_SIZE);
  ...
  return <div style={{ minHeight: "100vh", background: t.color.canvas }}>
    ...
    <div className="space-y-5 p-4 md:p-8">
      ...
      <div className="flex flex-wrap gap-2 border-b pb-2">{([['revenue','รายรับจากยอดขาย'],['trial','Trial Balance'],['valuation','Inventory Valuation'],['ledger','General Ledger']] as const).map(([key,label]) => <button key={key} className={`rounded px-4 py-2 text-sm ${activeTab === key ? 'bg-emerald-700 text-white' : 'border'}`} onClick={() => { setActiveTab(key); setReportSearchDraft(''); setReportSearch(''); }}>{label}</button>)}</div>
      <Card className="flex flex-wrap items-end gap-3 p-4"><label className="text-xs">ค้นหา {activeTab === 'revenue' ? 'เลขรายการ/ลูกค้า/ช่องทาง' : activeTab === 'trial' ? 'บัญชี' : activeTab === 'valuation' ? 'SKU/สินค้า/Lot' : 'Account'}<input className="mt-1 block h-9 min-w-64 rounded border px-2 text-sm" value={activeTab === 'ledger' ? accountDraft : reportSearchDraft} onChange={(e) => activeTab === 'ledger' ? setAccountDraft(e.target.value) : setReportSearchDraft(e.target.value)} placeholder="พิมพ์คำค้นหา" /></label><button className="h-9 rounded bg-emerald-700 px-3 text-sm text-white" onClick={() => { if (activeTab === 'ledger') setAccountFilter(accountDraft.trim()); else setReportSearch(reportSearchDraft.trim()); setRevenuePage(1); setTrialPage(1); setValuationPage(1); setLedgerPage(1); }}>ค้นหา</button><button className="h-9 rounded border px-3 text-sm" onClick={() => { setReportSearchDraft(''); setReportSearch(''); setAccountDraft(''); setAccountFilter(''); }}>ล้างตัวกรอง</button></Card>
      ...
      <Card className={`overflow-hidden ${activeTab === "revenue" ? "" : "hidden"}`}>
      ...
      <Card className={`overflow-hidden ${activeTab === "trial" ? "" : "hidden"}`}><div className="flex items-center justify-between border-b p-4"><div className="font-semibold">Trial Balance · {month}</div><div className="flex items-center gap-2"><Badge variant="secondary">{trial?.balanced ? "Balanced" : "Not balanced"}</Badge><button className="h-8 rounded border px-3 text-xs disabled:opacity-50" disabled={exportingPdf !== null} onClick={exportTrialPdf}>{exportingPdf === "trial" ? "กำลังสร้าง PDF..." : "Export PDF"}</button></div></div>
        ...
        </TableBody></Table><ReportPagination total={filteredTrial.length} page={trialPage} onPage={setTrialPage} /></Card>

      <Card className={`overflow-hidden ${activeTab === "valuation" ? "" : "hidden"}`}><div className="flex items-center justify-between border-b p-4"><div className="font-semibold">Inventory Valuation · {valuation?.totalQty ?? 0} ชิ้น</div><button className="h-8 rounded border px-3 text-xs disabled:opacity-50" disabled={exportingPdf !== null} onClick={exportValuationPdf}>{exportingPdf === "valuation" ? "กำลังสร้าง PDF..." : "Export PDF"}</button></div><Table><TableHeader><TableRow><TableHead>SKU / Product</TableHead><TableHead>Lot</TableHead><TableHead>หมดอายุ</TableHead><TableHead className="text-right">จำนวน</TableHead><TableHead className="text-right">ต้นทุน/ชิ้น</TableHead><TableHead className="text-right">มูลค่า</TableHead></TableRow></TableHeader><TableBody>{visibleValuationRows.map((row) => <TableRow key={`${row.SKU}-${row.Lot}`}><TableCell><div className="font-mono text-xs">{row.SKU}</div>{row.ProductName}</TableCell><TableCell className="font-mono">{row.Lot}</TableCell><TableCell>{row.ExpiryDate || "–"}</TableCell><TableCell className="text-right">{row.RemainingQty}</TableCell><TableCell className="text-right">{fmtBaht(row.UnitCost)}</TableCell><TableCell className="text-right font-semibold">{fmtBaht(row.Value)}</TableCell></TableRow>)}</TableBody></Table><ReportPagination total={filteredValuation.length} page={valuationPage} onPage={setValuationPage} /></Card>

      <Card className={`overflow-hidden ${activeTab === "ledger" ? "" : "hidden"}`}><div className="flex items-center justify-between border-b p-4"><div className="font-semibold">General Ledger · {month}</div><button className="h-8 rounded border px-3 text-xs disabled:opacity-50" disabled={exportingPdf !== null} onClick={exportLedgerPdf}>{exportingPdf === "ledger" ? "กำลังสร้าง PDF..." : "Export PDF"}</button></div><Table><TableHeader><TableRow><TableHead>วันที่ / Journal</TableHead><TableHead>เอกสารต้นทาง</TableHead><TableHead>บัญชี</TableHead><TableHead className="text-right">เดบิต</TableHead><TableHead className="text-right">เครดิต</TableHead></TableRow></TableHeader><TableBody>{visibleLedgerRows.map((row, index) => <TableRow key={`${row.JournalCode}-${index}`}><TableCell>{row.Date}<div className="font-mono text-xs">{row.JournalCode}</div></TableCell><TableCell>{row.SourceRef}<div className="text-xs text-muted-foreground">{row.SourceType}</div></TableCell><TableCell><span className="font-mono text-xs">{row.AccountCode}</span> {row.AccountName}</TableCell><TableCell className="text-right font-mono">{row.Debit ? fmtBaht(row.Debit) : "–"}</TableCell><TableCell className="text-right font-mono">{row.Credit ? fmtBaht(row.Credit) : "–"}</TableCell></TableRow>)}</TableBody></Table><ReportPagination total={filteredLedger.length} page={ledgerPage} onPage={setLedgerPage} /></Card>
    </div>
  </div>;
}
