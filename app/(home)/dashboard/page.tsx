...
export default function DashboardPage() {
  ...
  const yearOptions = useMemo(() => {
    ...
    salesOrders
      .map((order) => Number(order.date?.slice(0, 4)))
  ...
  // Helper to resolve channel with fallback to quotation lead source
  const resolveOrderChannel = (order: (typeof salesOrders)[number]) => {
    const directChannel = order.channel?.trim();
    if (directChannel && directChannel.toLowerCase() !== "manual") {
      return directChannel;
    }
    if (order.qtRef) {
      const qt = quotations.find(
        (q) => q.code === order.qtRef || q.id === order.quotationId,
      );
      if (qt?.leadSource) {
        const lead = qt.leadSource.trim().toLowerCase();
        if (lead === "tiktok") return "TikTok";
        if (lead === "shopee") return "Shopee";
        if (lead === "line") return "LINE";
        return qt.leadSource.trim();
      }
    }
  ...
  const channelMap = useMemo(() => {
    ...
    for (const order of periodOrders) {
      const ch = resolveOrderChannel(order);
      const cur = map.get(ch) || 0;
      map.set(ch, cur + order.amount);
    }
    return map;
  }, [periodOrders, quotations]);

  const channels = Array.from(channelMap.entries()).map(([name, rev]) => ({
    name,
    rev,
  ...
  const alerts: AlertItem[] = [
    ...
    ...pendingPRs
      ...
      .map((pr) => ({
        ...
        meta: pr.items && pr.items.length > 0 ? `${pr.items.length} รายการ` : "—",
        age: "—",
      })),
  ].slice(0, 6);
  ...
  return (
    <div className="min-h-screen bg-canvas" style={{ background: c.canvas }}>
      ...
      <PageBody t={t} maxWidth={1320}>
        <Card
          t={t}
          className="mb-4 p-[14px_18px] border border-border bg-card"
          style={{
            borderColor: "var(--erp-border)",
            background: "var(--erp-surface)",
          }}
        ...
        >
          {/* Header */}
          <div
            className="p-[22px_24px_18px] flex items-end justify-between border-b border-border"
            ...
            {[
              ...
              >
                <div className="flex items-center gap-2 mb-2.5">
                  ...
                  <span
                    className="text-[11px] font-medium tracking-[0.10em] uppercase text-muted-foreground"
                    style={{ color: "var(--erp-ink3)" }}
                  >
                    {s.label}
                  </span>
          ...
          <div className="p-[12px_16px_4px]">
            <div className="flex items-center gap-5 p-[4px_8px_8px_40px]">
              ...
              <div className="flex items-center gap-2">
                <span
                  className="w-4 h-0.5 opacity-50 rounded-sm inline-block"
                  style={{ background: c.ink2 }}
                />
                <span
                  className="text-[11px] text-muted-foreground"
                  style={{ color: "var(--erp-ink3)" }}
          ...
          {/* Weekly breakdown */}
          <div
            className="border-t border-border p-[14px_24px] grid grid-cols-4"
            style={{ borderColor: "var(--erp-border)" }}
          >
            {weeks.map((w, i) => (
              <div
              ...
              >
                <div
                  className="text-[10px] font-medium tracking-[0.08em] uppercase text-muted-foreground"
                  style={{ color: "var(--erp-ink3)" }}
                >
                  {w.label}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-6 mb-6">
          ...
          >
            ...
            <div
              ...
              style={{ color: c.ink3 }}
            >
              สัดส่วนยอดขายตามช่องทางประจำเดือน {periodLabel}
            </div>
        ...
        >
          <div className="flex flex-wrap items-end justify-between gap-4 mb-3">
            ...
            <div className="flex items-center gap-[18px]">
              <div className="flex items-center gap-1.5">
                <span
                  className="w-2.5 h-2.5 border border-border inline-block rounded-sm"
                  style={{
                    background: c.subtle,
                    borderColor: "var(--erp-border)",
                  }}
          ...
          <div
            className="grid grid-cols-6 gap-3.5 mt-3.5 pt-3.5 border-t border-border"
            style={{ borderColor: "var(--erp-border)" }}
          >
            {pnl6.map((d) => (
