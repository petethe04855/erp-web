              ...
              {rows.map((g) => {
                return (
                  <TableRow key={g.id}>
                    ...
                    <TableCell className="py-3.5 px-6">
                      <span style={{ fontSize: 12, color: c.ink2 }}>{g.dept}</span>
                    </TableCell>
                    <TableCell className="py-3.5 px-6">
                      <Mono t={t} size={12} color={c.ink2}>{g.date}</Mono>
                    </TableCell>
                    <TableCell className="py-3.5 px-6 text-right">
                      <Mono t={t} size={12} color={c.ink2}>1</Mono>
                    </TableCell>
                    <TableCell className="py-3.5 px-6">
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="sm:max-w-[520px] flex flex-col h-full p-0 bg-background">
          <SheetHeader className="p-6 border-b border-border flex-shrink-0">
            <SheetTitle className="text-base font-bold text-foreground">Issue Goods</SheetTitle>
            <SheetDescription className="text-xs text-muted-foreground mt-1">บันทึกการนำสินค้าออกจากคลัง</SheetDescription>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            ...
            <div className="grid gap-2">
              ...
              />
              {isOverStock && <div style={{ fontSize: 11, color: c.neg }}>เกินสต๊อกพร้อมเบิก ({available} ชิ้น)</div>}
            </div>
            ...
            <div className="grid gap-2">
              ...
              <select
                value={form.reason}
                onChange={e => setForm(f => ({ ...f, reason: e.target.value as GoodsIssueReason }))}
                className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs transition-colors focus-visible:outline-hidden focus-visible:ring-1 focus-visible:ring-ring"
              >
          ...
          <SheetFooter className="border-t border-border p-6 flex-shrink-0">
            <div className="flex justify-end gap-2 w-full">
              ...
              <Button onClick={handleSubmit} className="bg-[#0F6E58] text-white hover:bg-[#0F6E58]/90" disabled={!form.sku || isOverStock}>Save Issue</Button>
            </div>
          </SheetFooter>
        </SheetContent>
      </Sheet>
