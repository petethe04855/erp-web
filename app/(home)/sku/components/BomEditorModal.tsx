...
export default function BomEditorModal({
  ...
  const validRows = bomRows.filter(
    (r) => (r.componentType === "expense" || r.componentSku) && r.qty > 0,
  );
  const bomCost = validRows.reduce((sum, row) => {
    if (row.componentType === "expense")
      return sum + row.qty * row.unitCostOverride;
      ...
      >
        ...
        >
          <div>
            ...
            >
              กำหนดส่วนประกอบสำหรับสินค้าเซ็ต
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {validRows.length > 0 && (
          ...
          >
            ...
            >
              Virtual Stock Preview (ก่อนบันทึก)
            </div>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              {validRows
                .filter((r) => r.componentType !== "expense")
                .map((r) => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ fontWeight: 600 }}>{r.componentSku}</span>:{" "}
                      {avail} ÷ {r.qty} ={" "}
                      <span
                        style={{
                          fontWeight: 700,
                          color: canMake > 0 ? "#059669" : "#EF4444",
                        }}
                      >
                        {canMake} เซ็ต
                      </span>
              ...
              {Math.min(
                ...validRows
                  ...
                  .map((r) => {
                    ...
                    if (!cp) return 0;
                    let required = r.qty;
                    if (r.unit === "g" && cp.baseUnit === "kg")
                      required /= 1000;
                    if (r.unit === "kg" && cp.baseUnit === "g")
                      required *= 1000;
                    return Math.floor(
                      Math.max(0, cp.stock - cp.reservedQty) / required,
                    );
                  }),
              )}{" "}
              ชุด
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          >
            ต้นทุน BOM ต่อหน่วย: ฿
            {bomCost.toLocaleString("th-TH", { maximumFractionDigits: 2 })}
          </div>
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "105px 1fr 70px 75px 90px 36px",
              gap: 8,
              marginBottom: 6,
            }}
          >
          ...
          {bomRows.map((row, idx) => (
            ...
            >
              <select
                ...
                onChange={(e) =>
                  updateBomRow(idx, "componentType", e.target.value)
                }
              >
              ...
              <select
                ...
                onChange={(e) =>
                  updateBomRow(idx, "componentSku", e.target.value)
                }
                style={{
                  padding: "7px 10px",
                  borderRadius: 7,
              ...
              >
                ...
                {products
                  .filter((p) => !p.isBundle && p.sku !== bomSku)
                  .map((p) => (
              ...
              <input
                ...
                onChange={(e) =>
                  updateBomRow(idx, "unitCostOverride", +e.target.value)
                }
              />
              <button
