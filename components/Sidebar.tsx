...
const NAV_STATIC: NavEntry[] = [
  ...
  { label: "BOM", subTH: "สูตรและต้นทุนสินค้า", href: "/bom" },
  ...
  { label: "Production Run", subTH: "บันทึกการผลิต", href: "/production-run" },
  { section: "Finance" },
  ...
  { label: "TikTok Products", subTH: "TikTok products", href: "/tiktok-products" },
  { section: "System" },
...
function filterNav(
  ...
  for (const entry of entries) {
    if ("section" in entry) {
      ...
      const allowed =
        allowedHrefs === "*" ||
        (Array.isArray(allowedHrefs) && allowedHrefs.includes(entry.href));
...
export default function Sidebar() {
  ...
  const pendingLiveSessions = liveSessions.filter(
    (s) => s.status === "Pending",
  ).length;

  const NAV_WITH_BADGES: NavEntry[] = NAV_STATIC.map((item) => {
    if ("section" in item) return item;
    if (item.href === "/returns" && pendingReturns > 0)
      return { ...item, badge: pendingReturns };
    if (item.href === "/live-sessions" && pendingLiveSessions > 0)
      return { ...item, badge: pendingLiveSessions };
    return item;
  });

  const allowedHrefs = currentUser?.role
                    ...
                    return (
                      <Link
                        ...
                        style={{
                          ...
                          borderRadius: active
                            ? `0 ${Math.max(t.radius - 2, 0)}px ${Math.max(t.radius - 2, 0)}px 0`
                            : 0,
                          textDecoration: "none",
